...
export class ScoringSystem {
  ...
  public UpdateRatings(
    ...
    playerA.rating = Math.round(
      playerA.rating + K * (normScoreA - expectedScoreA),
    );
    playerB.rating = Math.round(
      playerB.rating + K * (normScoreB - expectedScoreB),
    );

    playerA.playerScore = Math.round(
      playerA.playerScore + K * (normScoreA - expectedScoreA),
    );
    playerB.playerScore = Math.round(
      playerB.playerScore + K * (normScoreB - expectedScoreB),
    );
  }

  public CalculateExpectedScore(playerA: any, playerB: any) {
    return (
      1.0 / (1.0 + Math.pow(10, (playerB.rating - playerA.rating) / 400.0))
    );
  }
  ...
  public UpdateRankPoints(players: any[]) {
    players.forEach((player) => {
      ...
      console.log(
        `Player ${player.userId} earned ${player.playerScore} rank points. Total rank points: ${player.rankpoints} with rating: ${player.rating}`,
      );
    });
  }

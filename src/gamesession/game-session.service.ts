...
export class GameSessionService {
  ...
  async createGameSession(
    ...
    await Promise.all(
      playerIds.map(async (playerId) => {
        await this.userModel.findByIdAndUpdate(playerId, {
          lastGameSession: createdGameSession._id,
        });
      }),
    );
  ...
  async updatePlayerStats(
    sessionId: string,
    playerStats: { playerId: string; kills: number; deaths: number }[],
  ) {
    try {
      ...
      for (const stat of playerStats) {
        const player = gameSession.players.find(
          (p) => p.userId.toString() === stat.playerId,
        );
        if (!player) {
          throw new Error(
            `Player with ID ${stat.playerId} not found in game session`,
          );
        }
      ...
      const playersData = gameSession.players
        .map((p) => {
          const user = userIdMap[p.userId.toString()];

          if (!user) {
            console.warn(
              `User with ID ${p.userId.toString()} not found in user map`,
            );
            return null;
          }
          return {
  ...
  async getLastSessionByUserId(userId: string) {
    ...
    const lastSession = await this.gameSessionModel.findById(

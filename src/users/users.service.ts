...
export class UsersService {
  ...
  async updateUserRank(updateUserDto: UpdateUserRankDto): Promise<User> {
    ...
    return await this.userModel.findByIdAndUpdate({ _id }, updateData, {
      new: true,
      upsert: true,
    });
  }
  ...
  async getUserRank(userId: string): Promise<any> {
    ...
    const higherRankCount = await this.userModel
      .countDocuments({ rankpoints: { $gt: rankpoints } })
      .exec();
  ...
  async equipItem(userId: string, itemId: string): Promise<any> {
    ...
    if (itemId === '0') {
      ...
      const unequipPromises = user.inventory
        .filter((i) => i.equipped)
        .map(async (i) => {
          const itemInInventory = await this.itemModel
            .findById(i.itemId)
            .exec();
          if (itemInInventory) {
            i.equipped = false;
  ...
  async getEquippedItemsForUsers(userIds: Types.ObjectId[]): Promise<any[]> {
    ...
    const itemIds = users.flatMap((user) =>
      user.inventory.map((item) => item.itemId),
    );

    const items = await this.itemModel.find({ _id: { $in: itemIds } }).exec();

    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    const itemDictionary = items.reduce(
      (dict, item) => {
